  ...
  <div>
    ...
    >
      ...
      <el-table-column label="修改时间" width="300">
        <template #default="scope"
          ><div v-if="scope.row.fileType != 'Dir'">{{ scope.row.lastModifyTime }}KB</div></template
        >
      </el-table-column>
      ...
      <el-table-column width="200">
        <template #default="scope"
          ><div class="op-btn" v-if="scope.row.fileType != 'Dir'">
            <downBtn :path="scope.row.filePath" :fileName="scope.row.fileName"> </downBtn></div
        ></template>
      </el-table-column>
    </el-table>
    ...
    <div v-if="total <= 0">
      <el-empty description="这里空空如野，快去登录上传文件吧" />
    </div>
  </div>
</template>
...
import { Folder, Download, Upload, Delete } from "@element-plus/icons-vue"
